...
<body>
	...
	<div class="main">
		...
		<div class="center" style="font-size: 25px; text-align: center;">
			...
			<form action="/Aerodrom/putnik/filtrirajRezervacije" method="get">
				<table>
					<tr>
						<td>Destinacija:</td>
			...
			<c:if test="${primenjenFilter}">
				...
				<a href="/Aerodrom/putnik/prikazRezervacija">Resetuj filtere</a> <br>
			</c:if>
			...
			<c:if test="${!empty karte}">
				...
				<table border="1">
					...
					<c:forEach items="${karte}" var="k">
						<tr>
							<td>
								<a href="/Aerodrom/putnik/getLetInfo?idLet=${k.let.idLet}">${k.let.aerodrom1.mesto} - ${k.let.aerodrom2.mesto}</a>
							</td>
							...
							<td>${k.datum}</td>
							<td>
								<a href="/Aerodrom/putnik/izbrisiRezervaciju?idKarta=${k.idKarta}">Potvrdi</a>
							</td>
							<td>
								<a href="/Aerodrom/putnik/dodajRecenziju?idKompanija=${k.let.kompanija.idKompanija}">Dodaj</a>
							</td>
						</tr>
					</c:forEach>
				</table>
